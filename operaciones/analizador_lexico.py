...
def sub_operaciones(i, j ,arreglo, configuracion, valor):
     for dato in arreglo[configuracion][i][valor]:
        ...
        for nombre in nombres_2:
             if nombre.lower() not in inicio2:
        ...
        if len(nombres_2) == 2:
            ...
            if isinstance(arreglo[configuracion][i][valor][j]["valor1"], list):
                sub_valor1 = sub_sub_operaciones(i,j,arreglo,configuracion,valor,"valor1")
            else:
        ...
        if len(nombres_2) == 3:
            ...
            if isinstance(arreglo[configuracion][i][valor][j]["valor2"], list):
                sub_valor2= sub_sub_operaciones(i,j,arreglo,configuracion,valor,"valor2")
            else:
...
def analizar(texto):
    ...
    for atributo in atributos:
        if atributo.lower() not in inicio_3:
                ...
                for tipo in datos_ingresados["operaciones"]:
                    ...
                    if len(nombres) == 2:
                        ...
                        if isinstance(datos_ingresados[configuracion][i]["valor2"], list):
                            j=0
                            nuevo_valor2 = sub_operaciones(i,j,datos_ingresados,configuracion, "valor2")
    ...
    if len(errores_validados["Errores"]) < 1:
        ...
        notificacion.geometry("400x150")
        etiqueta = tkinter.Label(notificacion, text="Archivo analizado con Exito, no se han encontrado errores")
        etiqueta.pack(padx=20, pady=20)
        boton_cerrar = tkinter.Button(notificacion, background="#FF1919",text="Cerrar", command=notificacion.destroy)
        boton_cerrar.pack(pady=10)
        boton_cerrar.config(height=2,width=10)
        ...
        etiqueta = tkinter.Label(notificacion, text="Se ha completado el analisis; errores encontrados, por favor pulse el botón 'Ver Errores' para más información")
        etiqueta.pack(padx=20, pady=20)
        boton_cerrar = tkinter.Button(notificacion, background="#FF1919",text="Cerrar", command=notificacion.destroy)
        boton_cerrar.pack(pady=10)
        boton_cerrar.config(height=2,width=10)