    ...
    if len(errores["Errores"]) < 1:
        ...
        try:
            with open("RESULTADOS_202100543.json", "w", encoding='utf-8') as archivo:
                json.dump(analizador_lexico.errores_validados, archivo, indent=4, ensure_ascii=False)
...
opciones_ayuda.add_command(label="Manual Tecnico", command=lambda: manual_tecnico())
opciones_ayuda.add_command(label="Manual de Usuario", command=lambda: manual_usuario())
